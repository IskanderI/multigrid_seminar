...
while err >= tol || it >= 1e7
    ...
    xold = x;
    it  = it + 1;
    if mod(it,round(P.nx^(1/2))) == 0
    figure(1),plot(P.x,x,LineWidth=3);title('Temperature');drawnow
    figure(2),plot(P.x,r,LineWidth=3);title(['Residuals, iterations: ',num2str(it)]);drawnow
    end
end
